...
if (isset($_POST['ChangeProfile'])) {
  ...
  $Qry = "UPDATE rpos_super_admin SET superadmin_name =?, superadmin_email =? WHERE superadmin_id =?";
  $postStmt = $mysqli->prepare($Qry);
  //bind paramaters
  $rc = $postStmt->bind_param('sss', $superadmin_name, $superadmin_email, $superadmin_id);
  $postStmt->execute();
...
if (isset($_POST['changePassword'])) {
  ...
  if (!$error) {
    $superadmin_id = $_SESSION['superadmin_id'];
    $sql = "SELECT * FROM rpos_superadmin   WHERE superadmin_id = '$superadmin_id'";
    $res = mysqli_query($mysqli, $sql);
    if (mysqli_num_rows($res) > 0) {
      $row = mysqli_fetch_assoc($res);
      if ($old_password != $row['superadmin_password']) {
        ...
        $query = "UPDATE rpos_superadmin SET  superadmin_password =? WHERE superadmin_id =?";
        $stmt = $mysqli->prepare($query);
        //bind paramaters
        $rc = $stmt->bind_param('ss', $new_password, $superadmin_id);
        $stmt->execute();
...
<body>
  ...
  <div class="main-content">
    ...
    $ret = "SELECT * FROM  rpos_super_admin  WHERE superadmin_id = '$superadmin_id'";
        ...
        <div class="container-fluid d-flex align-items-center">
          <div class="row">
            <div class="col-lg-7 col-md-10">
              <h1 class="display-2 text-white">Hello <?php echo $superadmin->superadmin_name; ?></h1>
            ...
            <div class="card card-profile shadow">
              ...
              <div class="card-body pt-0 pt-md-4">
                ...
                <div class="text-center">
                  <h3>
                    <?php echo $superadmin->superadmin_name; ?></span>
                  </h3>
                  <div class="h5 font-weight-300">
                    <i class="ni location_pin mr-2"></i><?php echo $superadmin->superadmin_email; ?>
                  </div>
                </div>
              </div>
            </div>
                    ...
                    <div class="row">
                      <div class="col-lg-6">
                        <div class="form-group">
                          ...
                          <input type="text" name="superadmin_name" value="<?php echo $superadmin->superadmin_name; ?>" id="input-username" class="form-control form-control-alternative" ">
                      </div>
                    </div>
                    <div class=" col-lg-6">
                          <div class="form-group">
                            <label class="form-control-label" for="input-email">Email address</label>
                            <input type="email" id="input-email" value="<?php echo $superadmin->superadmin_email; ?>" name="superadmin_email" class="form-control form-control-alternative">
                          </div>
                        </div>
