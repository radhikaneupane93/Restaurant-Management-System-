...
if (isset($_POST['ChangeProfile'])) {
  ...
  if (empty($_POST["waiter_phoneno"]) || empty($_POST["waiter_name"]) || empty($_POST['waiter_email'])) {
    ...
    $postQuery = "UPDATE rpos_waiter SET waiter_name =?, waiter_phoneno =?, waiter_email =?, waiter_password =? WHERE  waiter_id =?";
    $postStmt = $mysqli->prepare($postQuery);
    //bind paramaters
    $rc = $postStmt->bind_param('sssss', $waiter_name, $waiter_phoneno, $waiter_email, $waiter_password, $waiter_id);
    $postStmt->execute();
...
if (isset($_POST['changePassword'])) {
    ...
    if (!$error) {
        $waiter_id = $_SESSION['waiter_id'];
        $sql = "SELECT * FROM rpos_waiter   WHERE waiter_id = '$waiter_id'";
        $res = mysqli_query($mysqli, $sql);
        if (mysqli_num_rows($res) > 0) {
            $row = mysqli_fetch_assoc($res);
            if ($old_password != $row['waiter_password']) {
                ...
                //Insert Captured information to a database table
                $query = "UPDATE rpos_waiter SET  waiter_password =? WHERE waiter_id =?";
                $stmt = $mysqli->prepare($query);
                //bind paramaters
                $rc = $stmt->bind_param('si', $new_password, $waiter_id);
                $stmt->execute();
...
<body>
    ...
    <div class="main-content">
        ...
        $ret = "SELECT * FROM  rpos_waiter  WHERE waiter_id = '$waiter_id'";
                ...
                <div class="container-fluid d-flex align-items-center">
                    <div class="row">
                        <div class="col-lg-7 col-md-10">
                            <h1 class="display-2 text-white">Hello <?php echo $waiter->waiter_name; ?></h1>
                        ...
                        <div class="card card-profile shadow">
                            ...
                            <div class="card-body pt-0 pt-md-4">
                                ...
                                <div class="text-center">
                                    <h3>
                                        <?php echo $waiter->waiter_name; ?></span>
                                    </h3>
                                    <div class="h5 font-weight-300">
                                        <i class="fas fa-envelope mr-2"></i><?php echo $waiter->waiter_email; ?>
                                    </div>
                                    <div class="h5 font-weight-300">
                                        <i class="fas fa-phone mr-2"></i><?php echo $waiter->waiter_phoneno; ?>
                                    </div>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <div class="row">
                                            <div class="col-lg-6">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="waiter_name" value="<?php echo $waiter->waiter_name; ?>" id="input-username" class="form-control form-control-alternative" ">
                                                </div>
                                            </div>
                                            <div class=" col-lg-6">
                                                <div class="form-group">
                                                    <label class="form-control-label" for="input-email">Phone Number</label>
                                                    <input type="text" id="input-email" value="<?php echo $waiter->waiter_phoneno; ?>" name="waiter_phone" class="form-control form-control-alternative">
                                                </div>
                                            </div>
                                            <div class="col-lg-12">
                                                <div class="form-group">
                                                    <label class="form-control-label" for="input-email">Email address</label>
                                                    <input type="email" id="input-email" value="<?php echo $waiter->waiter_email; ?>" name="waiter_email" class="form-control form-control-alternative">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-lg-12">
