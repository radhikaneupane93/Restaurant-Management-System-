...
if (isset($_POST['make'])) {
  ...
  if (empty($_POST["order_code"]) || empty($_POST["customer_name"]) || empty($_GET['prod_price'])) {
    ...
    $postQuery = "INSERT INTO rpos_orders (order_code, customer_name, prod_name, prod_price,prod_qty) VALUES(?,?,?,?,?)";
    $postStmt = $mysqli->prepare($postQuery);
    //bind paramaters
    $rc = $postStmt->bind_param('sssss', $order_code, $customer_name, $prod_name, $prod_price, $prod_qty, );
    $postStmt->execute();
                ...
                <div class="form-row">
                  ...
                  <div class="col-md-4">
                    ...
                    <select class="form-control" name="customer_name" id="custName" onChange="getCustomer(this.value)">
                      ...
                      //--------------------------------------------------------------------------------------------------------
                      //Load All Customers
                      $ret = "SELECT * FROM  rpos_booking ";
                      $stmt = $mysqli->prepare($ret);
                      $stmt->execute();
                      $res = $stmt->get_result();
                      while ($booking = $res->fetch_object()) {
                      ?>
                        <option><?php echo $booking->booking_name; ?></option>
                      <?php } ?>
                    </select>
                ...
                ?>
                  <div class="form-row">
                    ...
                    <div class="col-md-6">
                      ...
                      <input type="text" readonly name="prod_price" value=" <?php echo $prod->prod_name; ?>" class="form-control">
                    </div>
